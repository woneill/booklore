...
public class MobileAuthorService {
    ...
    public MobilePageResponse<MobileAuthorSummary> getAuthors(
            Integer page,
            Integer size,
            String sortBy,
            String sortDir,
        ...
        List<MobileAuthorSummary> summaries = results.stream()
                .map(row -> {
                    ...
                    return MobileAuthorSummary.builder()
                            .id(author.getId())
                            .name(author.getName())
                            .asin(author.getAsin())
        ...
        if (hasPhoto != null) {
            ...
            return MobilePageResponse.of(summaries, pageNum, pageSize, filteredTotal);
        }
    ...
    @Transactional(readOnly = true)
    public MobileAuthorDetail getAuthorDetail(Long authorId) {
        BookLoreUser user = authenticationService.getAuthenticatedUser();
        Set<Long> accessibleLibraryIds = getAccessibleLibraryIds(user);
        ...
        return MobileAuthorDetail.builder()
                .id(author.getId())
                .name(author.getName())
